...
Last evidence update: **2026-07-22**. See the
...
| 1. Tenancy, identity, authorization      | IN_PROGRESS | Local login/session security, trusted verified-domain tenant resolution, Redis-backed auth rate limits, deny-by-default permission enforcement, protected tenant configuration, denied-action audit, refresh-race handling, and cross-tenant bearer rejection are unit- and live-tested. | Add isolated CI database/audit assertions, role and user administration, accessible sign-in/session UI, password recovery and email verification, MFA/recovery codes, and provider adapters. |
...
Finish Phase 1. The next vertical slice is documented under
