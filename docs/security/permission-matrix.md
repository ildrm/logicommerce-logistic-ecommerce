...
| `international.reference.manage` | Allow            | Deny                | Read-only          | Deny               | Allow with context    |
| `transport.document.manage`      | Allow            | Deny                | Hub operator       | Read-only          | Allow with context    |
| `customs.manage`                 | Allow            | Deny                | Read assigned      | Read-only          | Allow with context    |
| `insurance.use`                  | Allow            | Own policies/claims | Deny               | Read-only          | Allow with context    |
| `insurance.manage`               | Allow            | Deny                | Deny               | Policy             | Allow with context    |
| `insurance.claims.manage`        | Allow            | Deny                | Deny               | Insurance adjuster | Allow with context    |
| `logistics.network.read`         | Allow            | Deny                | Hub operator       | Read-only          | Allow with context    |
| `logistics.network.manage`       | Allow            | Deny                | Hub operator       | Deny               | Allow with context    |
| `logistics.handling.manage`      | Allow            | Deny                | Hub operator       | Deny               | Allow with context    |
| `logistics.consolidation.manage` | Allow            | Deny                | Hub operator       | Read-only          | Allow with context    |
| `postal.use`                     | Allow            | Own items           | Deny               | Deny               | Allow with context    |
| `postal.operate`                 | Allow            | Deny                | Postal operator    | Deny               | Allow with context    |
| `postal.admin`                   | Allow            | Deny                | Deny               | Deny               | Allow with context    |
...
The seeded `hub-operator` role receives network read/manage, handling,
consolidation, and transport-document permissions. The `postal-operator` role
receives postal operations and network read. The `insurance-adjuster` role
receives claims management and network read. None receives tenant, identity,
quotation, payment, or broad finance administration.
